...
export async function postPortfolioBriefing(body, session) {
  ...
  const res = await fetch('/api/analysis/portfolio-briefing', {
    method: 'POST',
    headers,
  ...
  try {
    ...
    throw new Error(`Portfolio briefing API returned non-JSON (${res.status})`)
  }
